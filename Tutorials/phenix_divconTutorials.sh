...
echo "Tutorial #1 (Running Phenix/DivCon without the qbphenix wrapper): 1IG3"
rm -rf ${WORKDIR}/NakedPHENIX ; mkdir -p ${WORKDIR}/NakedPHENIX ; cd ${WORKDIR}/NakedPHENIX

wget http://downloads.quantumbioinc.com/media/tutorials/XModeScore/1ig3.pdb
wget http://downloads.quantumbioinc.com/media/tutorials/XModeScore/1ig3-sf.cif

${PHENIX}/build/bin/phenix.ready_set 1ig3.pdb add_h_to_water=true

${PHENIX}/build/bin/phenix.refine 1ig3.updated.pdb 1ig3-sf.cif 1ig3.ligands.cif qblib=True qblib_method=pm6 qblib_region_selection="chain A and resname VIB and resid 502" qblib_region_radius=3.0 qblib_buffer_radius=3.0 qblib_np=4
...
$QBHOME/bin/qbphenix --dataFile 3ix1-sf.cif --pdbFile 3ix1.pdb --selection "chain A resname NFM resid 401"  --phenixOptions "main.number_of_macro_cycles=1" --protonation MOE --qmMethod pm6 --region-radius 3.0 --buffer-radius 2.5 --Nproc 4
...
$QBHOME/bin/qbphenix --pdbID 3ix1 --selection "chain A resname NFM resid 401" --phenixOptions "main.number_of_macro_cycles=1" --protonation ReadySet  --qmMethod pm6 --region-radius 3.0 --buffer-radius 2.5  --Nproc 4
...
$QBHOME/bin/qbphenix --dataFile 1lri-sf.cif --pdbFile 1LRI.pdb --selection "chain A resname CLR resid 99"  --phenixOptions "main.number_of_macro_cycles=1" --protonation MOE --qmMethod pm6 --region-radius 3.0 --buffer-radius 2.5 --Nproc 4 
...
$QBHOME/bin/qbphenix --pdbID 1NAV --mmMethod amberff14 --qmMethod pm6 --selection "resname IH5" --np 4 --region-radius 3.0 --buffer-radius 0.0 --protonation MOE >& OUT.screen

echo "Tutorial #6 (using qbphenix execution script and MOE on covalently bound ligand): 3NCK"
rm -rf ${WORKDIR}/Protonation ; mkdir -p ${WORKDIR}/Protonation ; cd ${WORKDIR}/Protonation

$QBHOME/bin/qbphenix --pdbID 3NCK --selection "resname NFF" --phenixOptions "main.number_of_macro_cycles=1" --protonation MOE  --qmMethod pm6 --region-radius 3.0 --buffer-radius 2.5  --Nproc 4  --scriptName run
