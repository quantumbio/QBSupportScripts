...
${DIVCON} 5C3K-H_refine_001.pdb 5C3K-H_refine_001.mtz   \   # input files. Since MTZ file is provided, it is understood that density docking will be used
    ...
    --confSearch 1 LIGAND opt off                     \   # Use the input conformer (LIGAND keyword which references --ligand input), keep poses/enumerations w/in top 1 standard deviation, perform torsion opt
...
${DIVCON} 5C3K-H_refine_001.pdb 5C3K-H_refine_001.mtz   \   # input files. Since MTZ file is provided, it is understood that density docking will be used
    ...
    --ligand 4XF_A_402.pdb                              \   # This is a placed ligand so no novel compound required. 
    --mtcs 1 LIGAND opt off                           \   # Use the input conformer (LIGAND keyword which references --ligand input), keep poses/enumerations w/in top 1 standard deviation, perform torsion opt
...
${DIVCON} 5C3K-refined.pdb  5C3K-refined.mtz -O         \   # Use the prepared files for this analysis.
    -h amberff14sb pm6                                  \   # Use AMBERFF14SB for all MM and PM6 for all QM. For v1: all steps up to XModeScore should be done with MM and final XModeScore should be done with QM/MM
